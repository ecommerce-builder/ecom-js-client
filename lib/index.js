...
class Catalog {
  ...
  strDumpTree(category) {
    let output = `name: ${category.name}\t uri: ${category.uri}\tlft: ${category.lft}\t rgt: ${category.rgt}\n`;

    for (let i = 0; i < category.subcategories.length; i++) {
      let subcategory = category.subcategories[i];

      output += this.strDumpTree(subcategory);
    }
  ...
  build() {
    ...
    let context = new Category(head.name, head.uri, head.lft, head.rgt);

    for (let i = 1; i < this._nestedset.length; i++) {
      let cur = this._nestedset[i];
		  // console.log(`cur: ${cur}\n`);

      let n = new Category(cur.name, cur.uri, cur.lft, cur.rgt);
      n.setParent(context);
		  context.addCategory(n);

		  // It is a Product Category and the context needs moving.
		  if (cur.lft === cur.rgt - 1) {
			  // console.log(`${cur} is leaf`);
        // console.log(`context: ${context}`);
          ...
          context = (context => {
	          ...
	          while ((prev.rgt === context.rgt - 1) && (context.parent != null)) {
		          prev = context;
		          context = context.parent;
	          }
	          return context;
          })(context);
				  // console.log(`context: ${context} moveContext(context)`);
        }
		  } else {
			  context = n
			  // console.log(`context: ${context} (context = n)`);
		  }
    }
