...
class Cart {
  constructor(fetch, endpoint, token, cartUuid) {
    Object.assign(this, {
      _fetch: fetch,
      _endpoint: endpoint,
      _token: token,
...
class Address {
   ...
   * @param {Date}   modified
   */
  constructor(fetch, endpoint, token, addrUUID, typ, contactName, addr1, addr2, city, county, postcode, country, created, modified) {
    Object.assign(this, {
      _fetch: fetch,
      _endpoint: endpoint,
      _token: token,
...
class Customer {
   ...
   * @param {Date}   modified
   */
  constructor(fetch, endpoint, token, customerUUID, uid, email, firstname, lastname, created, modified) {
    Object.assign(this, {
      _fetch: fetch,
      _endpoint: endpoint,
      _token: token,
...
class EcomClient {
  ...
  async createCart() {
    try {
      ...
      let data = await res.json();
      return new Cart(this._fetch, this._endpoint, this._token, data.cart_uuid)
    } catch (err) {
      console.error(err);
      throw err;
    }
